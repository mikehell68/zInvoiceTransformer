// from: https://www.codeproject.com/Articles/17840/Simple-CSV-Parser-Reader-Function-Written-in-C
...
namespace zInvoiceTransformer
{
    ...
    {
        static char _fieldDelimiter = ',';
        static char _textDelimiter = '"';

        public CsvParser(char fieldDelimiter, char textDelimiter)
        {
        ...
        {
            if (chrInput == _textDelimiter)
            {
                return 0;
            }
            else if (chrInput == _fieldDelimiter)
            {
                return 1;
            }
            else
